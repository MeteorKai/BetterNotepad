...
interface ConfirmAppCloseDialogProps {
  unsavedTabs: Array<{ id: string; fileName: string }>;
  onSaveAll: () => void;
  onDiscardAll: () => void;
  onCancel: () => void;
}

export default function ConfirmAppCloseDialog({
  unsavedTabs,
  onSaveAll,
  onDiscardAll,
  onCancel,
}: ConfirmAppCloseDialogProps) {
  ...
  return (
    ...
    >
      <div
        className="relative overflow-hidden bg-elevated border border-line rounded-2xl shadow-2xl w-[480px] p-6"
      ...
      >
        ...
        <div className="relative flex items-center gap-2 mb-3">
          ...
          <h2 className="text-base font-semibold text-ink">{t("confirm.closeAppAskTitle")}</h2>
        </div>
        <p className="relative text-sm text-sub mb-3">
          {t("confirm.unsavedLead")}
        </p>
        <ul className="relative mb-6 max-h-40 overflow-y-auto bg-surface border border-line rounded-lg p-2 space-y-1">
          {unsavedTabs.map((tab) => (
            <li key={tab.id} className="text-sm text-ink truncate">
